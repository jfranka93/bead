...
var errorCollection = require('../models/nevjegy');
...
describe('UserTest', function () {
    before(function (done) {
        User.destroy({loginname: 'abcdef'}, function (err) {
            if (err) throw err;
            done();
        });
    });
    it('should create a new user', function () {
       return User.create({
        ...
        .then(function (user) {
            expect(user.loginname).to.equal('abcdef');
   ...
   it('should be able to find a user', function() {
    return User.findOneByLoginname('abcdef')
    .then(function (user) {
            expect(user.loginname).to.equal('abcdef');
            expect(bcrypt.compareSync('jelszo', user.password)).to.be.true;
            expect(user.surname).to.equal('Gipsz');
